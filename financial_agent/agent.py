...
def _sanity_fix_amounts(norm: dict, resp: AgentResponse) -> dict:
    """財務恆等式防呆（純程式、確定性）：營業利益/淨利不該 > 同年營收。
    偵測到（多半是模型把 億 當 billion 之類的 10x 單位錯）就試以 10 的次方校正回
    合理區間（取最小校正）；校正不出來就標 null（寧缺勿錯，避免報告/圖表用到錯值）。"""
    metrics = norm.get("metrics", {})
    revenue = {}  # 年 → 營收（百萬），當錨點
    for metric, ymap in metrics.items():
        if _role_of(metric) == "revenue":
            revenue = {yr: v for yr, v in (ymap or {}).items() if isinstance(v, (int, float))}
            break

    corrections = []
    for metric, ymap in metrics.items():
        if _role_of(metric) != "constrained":
            continue
        for yr, v in list((ymap or {}).items()):
            r = revenue.get(yr)
            if not isinstance(v, (int, float)) or not isinstance(r, (int, float)) or r <= 0 or v <= r:
                continue  # 無從比對或本來就合理
            fixed = next((round(v * s, 2) for s in _SANITY_SCALES if 0 < v * s <= r), None)
            ymap[yr] = fixed
            corrections.append({"指標": metric, "年": yr, "原換算值": v,
                                "校正後": fixed, "同年營收": r})
    if corrections:
