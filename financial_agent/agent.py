...
# 明確的「從資料庫查/抓」硬訊號：使用者直接指明要查 DB 時，即使 planner 被髒歷史
# （例如先前誤回「我無法連資料庫」）帶歪成 chat，也要強制走 database_query。
_DB_SIGNAL_RE = re.compile(r"資料庫|數據庫|data\s*base|teradata|\bDB\b", re.IGNORECASE)
_DB_VERB_RE = re.compile(r"查|抓|撈|找|取|拉|匯出|報表|樞紐|select|sql", re.IGNORECASE)
# 針對「先前結果」的追問才該回 chat；這些字樣出現時不要硬轉 DB
_FOLLOWUP_RE = re.compile(r"剛才|剛剛|上一[個則步道]|上次|前面那|先前那|show\s*出來|秀出來|解釋|改一下")


def _looks_like_db_query(user_prompt: str) -> bool:
    """使用者是否「明確」要從資料庫撈資料（同時出現 DB 名詞＋撈取動詞，且非追問）。"""
    t = user_prompt or ""
    if _FOLLOWUP_RE.search(t):
        return False
    return bool(_DB_SIGNAL_RE.search(t) and _DB_VERB_RE.search(t))
...
def planning_phase(user_prompt: str, file_registry: Dict[str, str],
    ...
    try:
        ...
        # 硬性覆蓋：明確的「從資料庫查/抓」請求，不論 planner 因髒歷史判成什麼，
        # 一律強制 database_query。修『DB 請求被前幾輪幻覺回覆汙染、誤判成 chat』的惡性循環。
        if intent != IntentType.DATABASE_QUERY and _looks_like_db_query(user_prompt):
            print(f"  ⚙️ 偵測明確 DB 查詢字樣：意圖 {intent.value} → database_query（覆蓋誤判）")
            intent = IntentType.DATABASE_QUERY
            confidence = max(confidence, 0.9)
            first_tool = "get_database_schema"
            requires_files = False
            reasoning = f"[系統覆蓋] 偵測到明確資料庫查詢字樣。原判定：{reasoning}"
