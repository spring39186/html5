    ...
    with st.chat_message("assistant"):
        with st.status("🧠 Agent 思考中…", expanded=True) as status:
            ...
            def _on_progress(msg: str):
                ...
                # 整段包 try/except：長任務執行中若使用者觸發 rerun（動到上傳元件/重新整理/再送出），
                # 舊執行緒的 UI 寫入會撞到 Streamlit 元件狀態競態（如 duplicate key）。
                # 這種純顯示失敗絕不能往外拋——否則會穿進後端工具迴圈，被當成「工具執行錯誤」
                # 汙染查詢結果（實際案例：kb_uploader_0 重複 key 導致 run_sql_query 全數誤判失敗）。
                try:
                    _steps.append(msg)
                    ...
                    raise
                except BaseException:  # noqa: BLE001  含 Streamlit rerun/duplicate-key 等 UI 競態
                    pass
