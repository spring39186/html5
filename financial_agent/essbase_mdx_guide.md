...
- **時間／月份怎麼查（最常踩雷，務必照做）**：
  - 月份在 **`Time` 維底下**（Time → 年 → H1/H2 → 季 → **月**）；outline 裡月份是**收合的、沒有列出成員名**。
  - ⚠️ **嚴禁自創月份成員名**：`[Time].[Jan]`、`[Period].[Jan]`、`[Time].[Feb]`…**都不存在**，會查不到或只回部分月份。**不要用英文月名或 1~12 去拼成員**。
  - 要「某年各月」一律用 **`Descendants([Time].[2018], 3, SELF)`**（往下第 3 層≈各月）或 **`Descendants([Time].[2018], LEAVES)`**（取最底葉＝各月）。要全 12 個月就用這個，不要逐月列舉。
  - **`Period`、`Year` 是屬性維**（Period＝H1/H2/Q…、**沒有** Jan–Dec；Year＝2007–2025），只用於**篩選/切片**（如 `WHERE ([Year].[2018])`），**不可**拿來當月份欄軸。
