...
- **資料涵蓋範圍**：2012–2018
- **重要提醒 / 限制**：目前**只有 2018 年**的資料可查；篩年度請用 **Time 維成員 `[Time].[2018]`**（放 `WHERE` 或軸上），**不要用 Year 屬性維 `[Year].[2018]`**——本 cube 用屬性維切片會得到非預期值（實測金額全變成 `1`）。
...
9. **篩年度用 Time 成員、不要用 Year 屬性維**：用 `[Time].[2018]`（放 `WHERE` 或軸上）；
   `WHERE ([Year].[2018])` 在本 cube 會回非預期值（金額全變 `1`）。
