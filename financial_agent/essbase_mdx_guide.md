...
- **時間／月份怎麼查（最常踩雷，務必照做）**：
  ...
  - ✅ **「某年 12 個月」就照抄這一個（欄軸放這個）**：`Descendants([Time].[2018], 3, SELF)` → 剛好 `2018/01`…`2018/12` 共 12 欄、乾淨。要單月就 `[Time].[2018/03]`。（**本 cube 不支援 `LEAVES`**，一律用 `3, SELF`；**層數務必是 `3`＝月**，`1`＝只到半年 H1/H2、`2`＝只到季 Q1–Q4。）
  - 🚫 **絕對不要用 `[Time].[2018].Members`**：在本 cube 它會跑出 **`Period` 屬性維的所有層級**（`01`–`12`＋`H1`/`H2`＋`Q1`–`Q4`＋根 `Period`＋一個空成員）全部混在一排，不是你要的 12 個月。也不要 `[Time].[2018].Children`（只到半年 `H1`/`H2`）。**要 12 個月＝`Descendants([Time].[2018], 3, SELF)`**。
  - ⚠️ **嚴禁自創英文月名**：`[Time].[Jan]`、`[Period].[Jan]`、`[Time].[Feb]`…**都不存在**。月份要寫 `[Time].[2018/01]`、`[Time].[2018/12]` 這種格式（**補零**）。
  - **`Period`、`Year` 是屬性維**（Period＝H1/H2/Q…/`01`–`12`；Year＝2007–2025），主要用於**篩選/切片**（如 `WHERE ([Year].[2018])`）；**不要把 `Period` 整棵拉上欄軸**（會混入 H/Q/根）。要月份欄一律用 `Time` 的 `Descendants([Time].[2018], 3, SELF)`。
  - ⚠️ **要「每個月都列出來」（含沒資料的月顯示 0）時，不要加 `NON EMPTY`**：Essbase 會在回傳前就把「整月皆 #Missing」的月份（如某年 1~4 月）整欄藏掉，後端就補不回來了。把 12 個月（`Descendants([Time].[2018], 3, SELF)`）放上軸、且**不要 NON EMPTY**，後端會自動把 #Missing 補成 0。
...
| **指標**、科目、度量、**金額**、**銷售額** | **`Measure`**（維度名是單數 `Measure`，**不是** `Measures`） | 💰 **「銷售金額／金額」就用 `[Measure].[Current]`（本期值）**。⚠️ **沒有** `Sales Amount`／`Sales`／`Amount` 這種成員！其餘：`OP`、各種 `… Variance`／`… %`／`… YTD`。不確定要不要指定度量時，**可以整個不寫**（會用預設度量）。 |
| **情境**、版本、實際、草稿、預算、預測 | **`Scenario`** | `Actual`／`Draft`／`ForecastV1…4` 等 |
| **時間**、年、季、月 | **`Time`** | 月＝`[Time].[2018/01]`…；各月＝`Descendants([Time].[2018], 3, SELF)` |
