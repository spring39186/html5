...
- **時間／月份怎麼查（最常踩雷，務必照做）**：
  ...
  - ✅ **「某年 12 個月」就照抄這一個（欄軸放這個）**：`Descendants([Time].[2018], LEAVES)` → 剛好 `2018/01`…`2018/12` 共 12 欄、乾淨。要單月就 `[Time].[2018/03]`。
  - 🚫 **絕對不要用 `[Time].[2018].Members`**：在本 cube 它會跑出 **`Period` 屬性維的所有層級**（`01`–`12`＋`H1`/`H2`＋`Q1`–`Q4`＋根 `Period`＋一個空成員）全部混在一排，不是你要的 12 個月。也不要 `[Time].[2018].Children`（只到半年 `H1`/`H2`）。**要 12 個月＝`Descendants([Time].[2018], LEAVES)`**。
  - ⚠️ **嚴禁自創英文月名**：`[Time].[Jan]`、`[Period].[Jan]`、`[Time].[Feb]`…**都不存在**。月份要寫 `[Time].[2018/01]`、`[Time].[2018/12]` 這種格式（**補零**）。
  - **`Period`、`Year` 是屬性維**（Period＝H1/H2/Q…/`01`–`12`；Year＝2007–2025），主要用於**篩選/切片**（如 `WHERE ([Year].[2018])`）；**不要把 `Period` 整棵拉上欄軸**（會混入 H/Q/根）。要月份欄一律用 `Time` 的 `Descendants([Time].[2018], LEAVES)`。
